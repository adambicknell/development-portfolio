...
export default function CaseStudiesPage() {
  ...
  return <section className="section"><div className="container"><div className="space-between"><div className="animate-fade-in-left"><span className="eyebrow">Commercial proof</span><h1 className="h1">Case studies</h1><p className="lead">Anonymised examples based on private commercial systems, internal platforms, SaaS work, secure access, cloud delivery, and production support.</p></div><div className="card animate-fade-in-right"><strong>What these show</strong><p className="muted">Secure access, internal tools, SaaS, cloud, authentication, React, Python, data, and production support.</p></div></div><SharedTaxonomyFilters query={query} onQueryChange={setQuery} selectedParent={parent} onParentChange={setParent} selectedSkill={skill} onSkillChange={setSkill} groupedSkills={skills} /><p className="muted">Showing {filtered.length} of {caseStudies.length} case studies.</p><div className='grid grid-2 animate-grid-2'>{filtered.map((item)=><CaseStudyCard key={item.slug} item={item} />)}</div></div><CompareDrawer /></section>;
}
