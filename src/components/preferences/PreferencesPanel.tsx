'use client';
import { focusOptions, type FocusOption } from '@/data/site';
...
function toFocusOption(value: string): FocusOption | null {
  if (!value) return null;
  return focusOptions.includes(value as FocusOption) ? (value as FocusOption) : null;
}
    ...
    <div className="overlay" onClick={() => dispatch(setPreferencesOpen(false))}>
      <section className="modal" role="dialog" aria-modal="true" aria-label="Preferences" onClick={event => event.stopPropagation()}>
        ...
        <div className="grid grid-2">
          ...
          <div className="card-soft"><h3>Portfolio focus</h3><select className="select" value={prefs.selectedFocus ?? ''} onChange={event => dispatch(setSelectedFocus(toFocusOption(event.target.value)))}><option value="">All</option>{focusOptions.map(option => <option key={option}>{option}</option>)}</select></div>
