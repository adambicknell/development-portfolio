    ...
    <div className="overlay" onClick={() => dispatch(setPreferencesOpen(false))}>
      <section className="modal" role="dialog" aria-modal="true" aria-label="Preferences" onClick={event => event.stopPropagation()}>
        ...
        <div className="grid grid-2">
          ...
          <div className="card-soft"><h3>Hiring focus</h3><select className="select" value={prefs.selectedFocus ?? ''} onChange={event => dispatch(setSelectedFocus(event.target.value || null))}><option value="">All</option>{focusOptions.map(option => <option key={option}>{option}</option>)}</select></div>
